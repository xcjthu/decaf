...
public class BuildSym extends Tree.Visitor {
    ...
    public void visitTopLevel(Tree.TopLevel program) {
        ...
        for (Tree.ClassDef cd : program.classes) {
            Class c = new Class(cd.sealed, cd.name, cd.parent, cd.getLocation());
        ...
        for (Tree.ClassDef cd : program.classes) {
            ...
            if (c.getParent() != null && c.getParent().sealed){
    ...
    public void visitVarStmt(Tree.VarStmt varStmt){
        Variable v = new Variable(varStmt.name, BaseType.VARUNDEFINE, varStmt.getLocation());
        Symbol sym = table.lookup(varStmt.name, true);
        if (sym != null) {
            if (table.getCurrentScope().equals(sym.getScope())) {
                issueError(new DeclConflictError(v.getLocation(), v.getName(),
                        sym.getLocation()));
    ...
    public void visitIfSub(Tree.IfSub ifSub){
        if (ifSub.statementBody != null) {
            //System.out.println("visit if sub in buildsym");
